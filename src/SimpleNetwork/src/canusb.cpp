...
public:
    ...
    void dumpDataFrames() { // 需要设定读取时间，否则无法进行位置控制
        while (program_running) {
            auto frame = receiveFrame(13); // 假设最大帧长度为32 (8)
            ...
            if ((frame[4]==0x43 && frame[5]==0x64 && frame[6]==0x60)||(frame[4]==0x43 && frame[5]==0x6C && frame[6]==0x60))
                if(frame[2]>=0x81 && frame[2]<=0x86)
                    printFrame(frame);
        }
    }
...
private:
    ...
    {
        // station_num_low=order.station_num[1];
        // station_num_high=order.station_num[0];

        // if (station_num_high==0x02) // read x,y motor 
        // {
        //     terminate_after = 1;
        //     run();
        // }
        //  else if (station_num_high=0x01) // read pinch motor
        //  {
        //     terminate_after = 1;
        //     data.clear();
        //     data.push_back(0x92);
