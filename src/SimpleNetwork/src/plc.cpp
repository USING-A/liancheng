...
public:
    CanUsbInterface(const std::string& device, int baudrate,ros::NodeHandle & nh) 
        : tty_device(device), tty_fd(-1), baudrate(baudrate),nh_(nh) {
            sub=nh_.subscribe("/PLCController_motor_order",5,&CanUsbInterface::callback,this);
            // sub_sw=nh_.subscribe("/Controller_switch_order",5,&CanUsbInterface::callback_sw,this);
            // read_timer = nh_.createTimer(ros::Duration(0.01), &CanUsbInterface::timerCallback, this);
            // sub_read=nh_.subscribe("/Read_motor",5,&CanUsbInterface::callback_read,this);
            pub_read=nh_.advertise<liancheng_socket::ReadOutput>("Read_motor_output",5);
            speed=CanUsbHelper::int_to_speed(500000); 
    ...
    // void readOnce() {
    //     int id = 0;
    //     if (!nh_.getParam("/nx_id", id)) {
    //         ROS_ERROR("Failed to get '/nx_id' parameter");
    //         return;
    //     }
        
    //     while (program_running) {
    //         auto frame = receiveFrame(24);
            
    //         if (frame.empty()) {
    //             ROS_DEBUG("Empty frame received!");
    //             continue;
    //         }

    //         // 检查帧格式
    //         if (frame.size() < 12) {
    //             ROS_DEBUG("Incomplete frame received!");
    //             continue;
    //         }

    //         // 检查是否为目标ID的位置帧
    //         if (frame[0] == 0xAA && frame[12] == 0x55 && frame[3] == 0x02 && frame[2] == (0x50 + id)) {
    //             int32_t x_position = (frame[7]<<24) | (frame[6]<<16) | (frame[5]<<8) | frame[4];
    //             nh_.setParam("/read_x_motor_position", x_position*0.001);
                
    //             // for(int i=0;i<sizeof(frame);i++)
    //             //     ROS_INFO("0x%2X",frame[i]);
    //             ROS_INFO("Successfully read x%d position: %f mm", id, x_position*0.001);
    //             // printf("\n\n");
    //             frame.clear();
    //             return;
    //         }
    ...
    //         // 打印非目标帧的调试信息
    //         ROS_DEBUG("Received frame with ID 0x%02X (expected 0x%02X)", frame[2], 0x50 + id);
    //     }
    ...
    // void dumpDataFrames() {
    //     uint8_t id = 0;
    //     nh_.getParam("/nx_id", id); // 获取电机ID

    //     while (program_running) {
    //         auto frame = receiveFrame(13); 
    ...
    //         if (frame.empty()) {
    //             ROS_WARN("Received an empty frame. Skipping...");
    //             continue; // 处理接收错误或空帧
    //         }

    //         // 过滤读取数据：电机编号——81-86 05；43 64 60 读取位置；43 6C 60 读取速度
    //         if (frame[4] == 0x43 && frame[5] == 0x64 && frame[6] == 0x60) {
    //             if (frame[2] == (0x80 + id)) { 
    //                 x_position = (frame[11]<<24) | (frame[10]<<16) | (frame[9]<<8) | frame[8]; // 读取位置
    //                 nh_.setParam('/read_x_motor_position', x_position*0.001); // 将位置转换为毫米
    //                 ROS_INFO("Read x motor position: %f mm", x_position*0.001);
    //             }
    //         }
...
private:
    ...
    void callback(const liancheng_socket::MotorOrder & order)
    {
        ...
        for (int i=0;i<order_num;i++)
        {
            ...
            order_form=order.form[i];

        if (station_num_high==0x02 && order_form==11) // control x,y motor(absolutely)
        {
            terminate_after = 1;
            data.clear();
            data.push_back(0xa4);
            ...
            int16_t pos_high=int16_t(order.pos[i]>>16);
            int16_t pos_low=int16_t(order.pos[i]&0x0000ffff);
        ...
        else if (station_num_high==0x02 && order_form==12) // control x,y motor(relatively)
        {
            terminate_after = 1;
            data.clear();
            data.push_back(0xa4);
         ...
         else if (station_num_high=0x01 && order_form==200) // control pinch motor(relatively)
         {
            terminate_after = 1;
            data.clear();
            data.push_back(0xa8);
            ...
            data.push_back(uint8_t(order.vel[i]&0x00ff));
            data.push_back(uint8_t(order.vel[i]>>8));
            int32_t pos_act = order.pos[i]*100; // 100 LSB -> 1 degree
            int16_t pos_high=int16_t(pos_act>>16);
            int16_t pos_low=int16_t(pos_act&0x0000ffff);
            data.push_back(uint8_t(pos_low&0x00ff));
            data.push_back(uint8_t(pos_low>>8));
            data.push_back(uint8_t(pos_high&0x00ff));
            data.push_back(uint8_t(pos_high>>8));
        ...
        else if (station_num_high=0x01 && order_form==199) // control pinch motor(absolutely)
         {
            terminate_after = 1;
            data.clear();
            data.push_back(0xa4);
         ...
         else if (station_num_high=0x01 && order_form==201) //write current postion as pinch motor's zero poistion
         {
            terminate_after = 1;
            data.clear();
            data.push_back(0x64);
            ...
            data.push_back(0x00);
            data.push_back(0x00);
            int16_t encoderOffset_high=int16_t(order.pos[i]>>16);
            int16_t encoderOffset_low=int16_t(order.pos[i]&0x0000ffff);
            data.push_back(uint8_t(encoderOffset_low&0x00ff));
            data.push_back(uint8_t(encoderOffset_low>>8));
            data.push_back(uint8_t(encoderOffset_high&0x00ff));
            data.push_back(uint8_t(encoderOffset_high>>8));
            run();
         }
        else if (station_num_high=0x03) // relay
         {
            terminate_after = 1;
            data.clear();
            data.push_back(uint8_t(order.vel_ac[i]&0x00ff)); // switch_num
            data.push_back(uint8_t(order.vel_ac[i]>>8));
            data.push_back(uint8_t(order.form[i])); // case_num
            data.push_back(0x00);
            data.push_back(0x00);
            data.push_back(0x00);
            data.push_back(0x00);
    ...
    //     terminate_after = 1;
    //     data.clear();
    //     data.push_back(uint8_t(order.switch_num&0x00ff));
    //     data.push_back(uint8_t(order.switch_num>>8));
    //     data.push_back(uint8_t(order.case_num));
    //     data.push_back(0x00);
    //     data.push_back(0x00);
    //     data.push_back(0x00);
    //     data.push_back(0x00);
    ...
    // {
    //     int station_num = order.station_num;
    //     if (station_num==0x02)
    //     {
    //         terminate_after = 1;
    //         read();
...
int main(int argc, char* argv[]) {
    try {
        ...
        ros::NodeHandle nh;
        std::string port;
        CanUsbInterface canInterface("/dev/ttyUSB1",CANUSB_TTY_BAUD_RATE_DEFAULT,nh);
